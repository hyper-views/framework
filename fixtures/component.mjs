...
export default ({state}) => main(
  ...
  svg(Boolean(state.hasSvg), () => [
    {xmlns: 'http://www.w3.org/2000/svg'},
    path({d: 'M2 2 2 34 34 34 34 2 z'})
  ]),
  div(Boolean(state.hasOnmount), () => ({
    onmount () {
      this.innerHTML = 'onmount set'
    }
  })),
  div(Boolean(state.hasOnupdate), () => ({
    onupdate () {
